...
fig1.add_trace(go.Scatter(
    ...
    mode="lines", name="Raw Index", line=dict(dash="dot", color="silver")
))
fig1.add_trace(go.Scatter(
    x=mom_df["date"], y=mom_df["MoM_smoothed"],
    mode="lines", name="Smoothed (3 mo EWMA)", line=dict(width=3, color="white")
))
...
st.download_button("Download Headline Data", data=csv_mom,
                   file_name="mom_scores.csv", mime="text/csv")
...
fig2.add_trace(go.Scatter(
    ...
    mode="lines+markers", name=selected, line=dict(color="silver")
))
...
st.download_button(f"Download {selected} Data",
                   data=csv_section,
